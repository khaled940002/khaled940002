...
function spawnEnemies() {
    setInterval(() => {
        const raidus = Math.random() * (30 -4) + 4 

        let x
        let y 

        if (Math.random() < 0.5 ) {
         x = Math.random() < 0.5 ? 0 - raidus : canvas.width + raidus 
         y= Math.random() * canvas.height
         x= Math.random() * canvas.width
         y = Math.random() < 0.5 ? 0 - raidus : canvas.height + raidus
        }
        ...
        enemies.push(new Enemy(x, y, raidus, color, velocity))
...
projectiles.push(
    new Projectile(canvas.width / 2,canvas.height /2, 5, 'gray',velocity )
    )
